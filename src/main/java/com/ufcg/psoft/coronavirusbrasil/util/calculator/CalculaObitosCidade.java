...
public class CalculaObitosCidade implements Calculador {
    ...
    public String calculaObitos(Sexo sexo, int faixaEtariaInicio, int faixaEtariaFim, String local) {
        ...
        for (Hospital h : this.hospitalRepository.findAll()) {
            ...
            for (Paciente p : obitos) {
                ...
                if (p.getSexo().equals(sexo) && dentroFaixaEtaria && p.getEndereco().getCidade().equals(local)) {
                    numeroDeObitos++;
                }
            }
        }
        return String.format("O número de óbitos para o sexo %s, dentro da faixa etária de %d aos %d anos, na cidade de" +
                " %s foi de %d pessoas.", sexo, faixaEtariaInicio, faixaEtariaFim, local, numeroDeObitos);
    }
}
